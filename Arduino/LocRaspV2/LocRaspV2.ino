  ...
  while (millis() - start < timeout) {
    while (modemSerial.available()) {
      ...
      if (resp.indexOf("*ATREADY: 1") != -1) {
        ...
        modemSerial.flush();
        delay(5000); // dejar que termine de iniciar
        sendAT("ATE0");
        sendAT("AT+CFUN=1");
        delay(2000);
        sendAT("AT+CGATT=1");
      ...
      if (resp.indexOf("+CGEV: ME PDN DEACT") != -1) {
        Serial.println("⚠️ Conexión de datos caída detectada");
        mqttConectado = false;
        return resp; // salir sin reintentar aquí
      }
...
bool conectarRed() {
  ...
  if (!ok) {
    Serial.println("⚠️ Esperando recuperación de red...");
    delay(1000);
  }
  return ok;
}
...
bool ensureGnssOn() {
  ...
  // 🕐 Esperar si la red está ocupada (evita bloqueos)
  ...
  // 🧠 Activar GNSS con reintento si falla
  for (int i = 0; i < 3; i++) {
    ...
    Serial.println("⚠️ GNSS no respondió, reintentando...");
...
String generarCSV(String hora, String lat, String lng, String alt, String hdopStr) {
  String csv = "Furgoubi," + hora + "," + lat + "," + lng + "," + alt + "," + hdopStr + ",";
...
bool publicar(String payload) {
  ...
  // 🔸 Verificar IP antes de publicar
  String ip = sendAT("AT+CGPADDR=1", "OK", 2000);
  if (ip.indexOf("0.0.0.0") != -1) {
    Serial.println("⚠️ Sin IP, reintentando conexión de datos...");
    conectarRed();
    mqttConectado = initMQTT();
  }

  // 🔸 Esperar estabilización tras conexión
  Serial.println("⌛ Esperando estabilización MQTT...");
  delay(1000);
...
void parkUnusedPins() {
  // Lista de los GPIO que SÍ usas en tu proyecto
  bool used[40] = {false};

  used[MODEM_RX] = true;  // 22
  used[MODEM_TX] = true;  // 23
  // Marca aquí otros que uses: I2C, SPI, LEDs, etc.

  for (int g = 0; g < 40; g++) {
    if (!isSafeGpio(g)) continue;
    if (used[g]) continue;

    // “Silenciar” = evitar flotantes
    pinMode(g, INPUT_PULLDOWN); // o INPUT_PULLUP si te encaja mejor
  }

  // UART RX del módem: en reposo debe quedar en HIGH (silencio)
...
void setup() {
  ...
  ensureGnssOn();
  lastCheck = millis();
  lastPublish = 0; // para permitir publicación inmediata
...
void loop() {
  ...
  if (millis() - ultimoFixValido > (10UL * 60 * 1000UL)) {
    Serial.println("⚠️ Más de 10 minutos sin fix, reiniciando GNSS...");
  ...
  if (now - lastCheck > 15000UL) {
    ...
    if (raw == "" || raw.indexOf("+CGNSSINFO:") == -1) {
      gnssFails++;
      if (gnssFails >= 3) {
        Serial.println("⚠️ GNSS parece colgado, reiniciando subsistema...");
        sendAT("AT+CGNSSPWR=0", "OK", 3000);
        delay(1000);
        sendAT("AT+CGNSSPWR=1", "OK", 5000);
        gnssFails = 0;
      ...
      gnssFails = 0; // reset contador si GNSS responde bien
    }
    ...
    // Detectar cambio de estado de movimiento
