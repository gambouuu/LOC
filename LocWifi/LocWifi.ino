...
const char* mqtt_user = "Nomada";                // 🔹 Nuevo usuario
const char* mqtt_pass = "gamboalocalizador";
...
String sendAT(const String& cmd, const String& expected = "OK", int timeout = 8000) {
  ...
  Serial.println("⚠️ Timeout esperando respuesta de: " + cmd);
  return resp;
}
...
void comprobarReinicioPorGPS() {
  if (millis() - ultimoFixValido > MAX_SIN_FIX) {
    Serial.println("⚠️ Reinicio: más de 20 min sin fix GPS válido");
    delay(1000);
    esp_restart();
  }
}
...
void loop() {
  ...
  if (now - lastCheck > 30000UL) {
    ...
    Serial.println("🛰️ GPS Fix válido");
